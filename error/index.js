...
errors = {
    ...
    logError : function(err, context, help) {
        ...
        if(_.isArray(err)){
            _.each(err, function(e){
                ...
                error.logError.apply(self, newArgs);
            });
            return;
        }
    ...
    handleGithubError : function(body, callback){
        var msgs = [body.message.red, '\n'];

        if(body.errors){
            _.each(body.errors, function(error){
                _.each(_.keys(error), function(key){
                    msgs.push(key.blue + ": " + error[key] + "; ");
                });
                msgs.push('\n');
            });
        }
        ...
        if(body.documentation_url){
            msgs.push(body.documentation_url.green, '\n');
        }
    ...
    renderErrorPage: function (code, err, req, res, next) {
        /*jshint unused:false*/
        var self = this,
            defaultErrorTemplatePath = path.resolve(workDirectory, 'user-error.hbs');
